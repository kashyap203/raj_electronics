...
import { FaShoppingBag, FaMapMarkerAlt, FaCreditCard, FaCheckCircle } from 'react-icons/fa';
...
const OrderDetailPage = () => {
  ...
  if (!order) return <div className="max-w-4xl mx-auto px-4 py-12 text-center text-gray-500">Order not found</div>;
    ...
    <div className="max-w-4xl mx-auto px-4 py-8">
      ...
      {justPlaced && (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 mb-6 flex items-center gap-3 animate-fade-in">
          ...
          <div>
            ...
            <p className="text-sm text-green-600">Thank you for shopping with Raj Electronics.</p>
          </div>
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 mb-6">
        ...
        <div className="flex items-center gap-3">
          <span className={`text-sm font-semibold px-3 py-1 rounded-full ${statusColors[order.status]}`}>
            {order.status}
          </span>
          {['Pending', 'Processing'].includes(order.status) && (
            <button onClick={handleCancel} disabled={cancelling} className="text-sm text-red-500 hover:text-red-700 border border-red-300 hover:border-red-500 px-3 py-1 rounded-full transition">
            ...
            {statusSteps.map((step, i) => (
              <div key={step} className="flex flex-col items-center relative z-10">
                <div className={`w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold border-2 transition ${
                  i <= currentStep ? 'bg-primary border-primary text-dark' : 'bg-white border-gray-300 text-gray-400'
                }`}>
                  {i < currentStep ? '✓' : i + 1}
                </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-2 space-y-3">
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <h2 className="font-bold text-gray-800 mb-4 flex items-center gap-2"><FaShoppingBag className="text-primary" /> Items Ordered</h2>
              ...
              {order.products.map((item, i) => (
                <div key={i} className="flex gap-4 pb-4 border-b border-gray-100 last:border-0 last:pb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {item.discount > 0 && <p className="text-xs text-green-600">{item.discount}% off applied</p>}
                  </div>
                  <p className="font-semibold text-sm shrink-0">{formatPrice(Math.round(item.price - item.price * item.discount / 100) * item.quantity)}</p>
                </div>
              ))}
            </div>
            ...
            <div className="border-t border-gray-200 mt-4 pt-4 space-y-2 text-sm">
              ...
              <div className="flex justify-between text-gray-600">
                <span>Shipping</span>
                <span className={order.shippingPrice === 0 ? 'text-green-600' : ''}>{order.shippingPrice === 0 ? 'FREE' : formatPrice(order.shippingPrice)}</span>
              </div>
        ...
        <div className="space-y-4">
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><FaMapMarkerAlt className="text-primary" size={14} /> Delivery Address</h3>
            {order.address && (
          ...
          <div className="bg-white rounded-2xl shadow-sm p-4">
            <h3 className="font-bold text-gray-800 mb-3 flex items-center gap-2"><FaCreditCard className="text-primary" size={14} /> Payment</h3>
